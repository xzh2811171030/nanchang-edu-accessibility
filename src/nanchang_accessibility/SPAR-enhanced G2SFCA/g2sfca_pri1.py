...
destination_file = r'D:\Xia\比赛\论文2\3\3.1\des.txt'
line_file = r'D:\Xia\比赛\论文2\3\3.1\lines.txt'  # 请确保该文件在同一目录下
output_file = r'D:\Xia\比赛\论文2\3\3.1\Nanchang_SPAR_Result.csv'
...
def calculate_g2sfca():
    ...
    # 关联格网需求量 (demand_pri)
    df_line = df_line.merge(df_origin[['Name', 'demand_pri']], 
                            left_on='O_ID', right_on='Name', how='left')
    
    # 计算每条线的加权需求量
    df_line['weighted_demand'] = df_line['demand_pri'] * df_line['G_weight']
    ...
    # 4. 保存结果
    output_file_jun = f"{file_root}_jun{file_ext}"
    ai_result.to_csv(output_file, index=False)
    print("-" * 30)
    print(f"✅ 计算完成！结果已保存至: {output_file_jun}")
